...
/**
 * Rand2Mutator is a class implementing DE/rand/2 mutation scheme over the
 * interface Mutator.
 *
 ...
 * @since 1.0
 */
public class Rand2Mutator implements Mutator {

    private final Execution exe;
    private static final int NUM_RANDOM_IND = 5;
     ...
     */
    public Rand2Mutator(Execution exe) {
        this.exe = exe;
    }
    ...
    public Individual mutateIndividual(Population p, Individual X, Individual... individuals) {
        ...
        for (int k = 0; k < D; k++) {
            V.setGene(k, r1[k] + F * (r2[k] - r3[k]) + F * (r4[k] - r5[k]));
        }
